...
const getLocalizationByCoordinator = async (latitude, longitude) => 
    await httpRequest.get(`${apis.sptrans.url}/Login/Autenticar?=${apis.sptrans.token}&p.py=${latitude}&p.px=${longitude}&format=json`);
    //await httpRequest.post(`${apis.sptrans.url}/Login/Autenticar?=${apis.sptrans.token}`);
